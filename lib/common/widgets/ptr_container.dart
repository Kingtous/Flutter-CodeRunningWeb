...
enum FooterStatus { PREPARE, START, LOADING, COMPLETE, RELEASE }

enum HeaderStatus { PREPARE, START, REFRESHING, COMPLETE, RELEASE }
...
abstract class RefreshHeaderState<T extends RefreshHeader> extends State<T>
    implements RefreshHeaderListener {
...
class RefreshHeaderImpl extends RefreshHeader {
  ...
  RefreshHeaderImpl(
      ...
      this.finishDelay: 1000})
      : super(
            key: key, refreshHeight: refreshHeight, finishDelay: finishDelay) {
...
abstract class LoadMoreFooterState<T extends LoadMoreFooter> extends State<T>
    implements LoadMoreFooterListener {
...
class LoadMoreFooterImpl extends LoadMoreFooter {
  ...
  LoadMoreFooterImpl(
      ...
      this.footerHeight: 70.0,
      this.finishDelay: 1000})
      : super(key: key, footerHeight: footerHeight, finishDelay: finishDelay) {
    assert(listener != null);
  }
...
class PtrGesturesHelper {
  ...
  ScrollController get createController => _scrollController == null
...
class PtrContainer extends StatefulWidget {
  ...
  State<StatefulWidget> createState() => PtrContainerState(
      header, body, footer, loadMoreListener, refreshListener, listBuilder);
}
...
class PtrContainerState extends State<PtrContainer> {
  ...
  PtrContainerState(this.header, this.body, this.footer, this.loadMoreListener,
        ...
        child: Listener(
          ...
          onPointerMove: (event) {
            ...
            if (ret > 0) {
              ...
              var scrollExtent =
                  helper.createController.position.maxScrollExtent;
              var result =
                  helper.createController.offset + (position - downY).abs();
